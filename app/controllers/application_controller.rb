class ApplicationController < ActionController::API
	...
	def test_greenhouse_service_token
	 	puts '--------------'
	 	params.require('atoken')
		@gserv = GreenhouseService.where(authentication_token: params[:atoken])
		if @gserv.empty?
			render json: {status: 'ERROR', message: 'Token not valid', data: 0}, status: :unprocessable_entity
			return false
		else
	 ...
	 def test_greenhouse_service_token_expiration
	 	puts '-------------------'
	 	# update_time = @user[0]['updated_at'].to_time
	 	# puts 'current-1min:'
	 	# puts Time.now - 1.hour
	 	...
	 	# puts update_time
	 	# if update_time < (Time.now - 1.hour)
	 	update_date = @gserv[0]['updated_at'].to_date
	 	puts 'current-10days:'
